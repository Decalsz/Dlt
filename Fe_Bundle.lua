...
local function createGui()
    ...
    -- Drag hitbox over header. This is reliable on Delta/mobile.
    local HeaderDrag = new("TextButton", {Parent=Main, Position=UDim2.new(0,0,0,0), Size=UDim2.new(1,-58,0,58), BackgroundTransparency=1, Text="", BorderSizePixel=0, Active=true, AutoButtonColor=false, ZIndex=115})

    local mainDragging = false
    local mainInput, mainStart, mainPos
    add(HeaderDrag.InputBegan:Connect(function(input)
    ...
    add(HeaderDrag.InputChanged:Connect(function(input)
    ...
    local function toggleMain()
        local now = os.clock()
        if now - lastIconTap < 0.22 then return end
        lastIconTap = now
    ...
    add(IconButton.MouseButton1Click:Connect(toggleMain))
    add(IconButton.MouseButton1Down:Connect(toggleMain))
    pcall(function() add(IconButton.Activated:Connect(toggleMain)) end)
    add(IconButton.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.Touch then toggleMain() end
    end))
