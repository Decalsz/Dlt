--[[
    Irenk Bundle + Emote Hub v8.3 - Info Saweria
...
local function createGui()
    ...
    pcall(function()
        for _, name in ipairs({"IrenkBundleAnimationHub", "IrenkBundleAnimationHubV2", "IrenkBundleAnimationHubV3", "IrenkBundleAnimationHubV4Saweria", "IrenkBundleAnimationHubV5CleanSaweria", "IrenkBundleAnimationHubV6CleanSaweria", "IrenkBundleAnimationHubV7OriginalSaweria", "IrenkBundleEmoteHubV8InfoSaweria", "IrenkBundleEmoteHubV83InfoSaweria"}) do
            local old = pg:FindFirstChild(name)
            if old then old:Destroy() end
        end
    end)
    ScreenGui = new("ScreenGui", {Name="IrenkBundleEmoteHubV83InfoSaweria", ResetOnSpawn=false, IgnoreGuiInset=true, DisplayOrder=999999, ZIndexBehavior=Enum.ZIndexBehavior.Global})
    ScreenGui.Parent = pg
    ...
    -- Persistent close button. Do not use page connection here, otherwise it dies when pages rerender.
    local CloseButton = new("TextButton", {Parent=Main, Position=UDim2.new(1,-48,0,12), Size=UDim2.new(0,34,0,32), BackgroundColor3=Theme.Red, BorderSizePixel=0, Text="X", TextColor3=Theme.Text, TextStrokeTransparency=1, TextSize=14, Font=Enum.Font.GothamBold, AutoButtonColor=true, ZIndex=120})
    corner(CloseButton, 8); stroke(CloseButton, Theme.Black, 1, 0)
    ...
    add(Connections, CloseButton.Activated:Connect(function()
        Alive=false; stopEmote(); disconnectList(Connections); disconnectList(PageConnections); if ScreenGui then ScreenGui:Destroy() end
    end))
    ...
    -- ROBUST DRAG SYSTEM (mouse + touch/mobile)
    -- Header is used to drag the main window. The floating icon can also be dragged.
    ...
    add(Connections, HeaderDrag.InputChanged:Connect(function(input)
    ...
    add(Connections, UserInputService.InputChanged:Connect(function(input)
        if not mainDragging or not mainDragInput then return end
        if input == mainDragInput or input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch then
    ...
    add(Connections, UserInputService.InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            mainDragging = false
            mainDragInput = nil
        end
    ...
    -- Open/close button. Use Activated only once so mobile doesn't double-trigger.
    local lastIconTap = 0
    ...
    local function toggleMain()
        local now = os.clock()
        if now - lastIconTap < 0.18 then return end
        lastIconTap = now
        Main.Visible = not Main.Visible
        if Main.Visible and not Body:FindFirstChildWhichIsA("GuiObject") then
            local ok, err = pcall(function() renderHome("Bundle") end)
            if not ok then
                warn("Irenk Bundle Hub open error: " .. tostring(err))
                status("Open error: " .. tostring(err), false)
            end
        end
    end
    ...
    add(Connections, IconButton.InputChanged:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch then
            iconDragInput = input
        end
    end))
    ...
    add(Connections, UserInputService.InputChanged:Connect(function(input)
        if not iconDragging or not iconDragInput then return end
        if input == iconDragInput or input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch then
    ...
    add(Connections, UserInputService.InputEnded:Connect(function(input)
        ...
        if iconDragging and not iconDragMoved then
            toggleMain()
        end
        iconDragging = false
        iconDragMoved = false
